...
def handle_each_email(service, message_id, logger) -> tuple:
    ...
    if single_email.get("subject") in constants.SUBJECTS:
        # workout which notification we are dealing with and use the correct
        # regular expression string
        if single_email.get("from") == constants.FROM_BUS:
            data = patterns.findMatches(single_email.get_content(),
                                        patterns.BUS_DATA)
            datetime = patterns.findMatches(single_email.get_content(),
                                            patterns.BUS_DATE_TIME)
            # Merge data and datetime into a single dictionary
            data.update(datetime)
            notifier = "BUS"
        elif single_email.get("from") == constants.FROM_KIDZDUO:
            data = patterns.findMatches(single_email.get_content(),
                                        patterns.KIDZDUO_ENTEREXIT)
            notifier = "KIDZDUO"
        elif single_email.get("from") == constants.FROM_GATE:
            data = patterns.findMatches(single_email.get_content(),
                                        patterns.GATE_DATETIME)
            notifier = "GATE"
        else:
            # This needs to log. Means failed to match sender.
            # if notifier is None:
            logger.warning(f"Failed to process message_id: {message_id} "
                           f"Matched Subject: {single_email.get('subject')} "
                           f"Sender not matched: {single_email.get('from')}")
            pass
        return notifier, data
    else:
        # Not an expected subject line. Ignore this email
        logger.info("This is not a notifcation.")
        logger.info(f"sender: {single_email.get('from')}\n\t")
        logger.info(f"subject: {single_email.get('subject')}")
        return notifier, data
